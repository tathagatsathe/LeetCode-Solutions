...
class Trie:
    ...
    def searchPrefix(self, key):
        curr = self.root
        # print('curr: ',curr.index, ' key: ', key)
        # print(curr.children[0].index if curr!=None and curr.children[0]!=None else None)
        # print(curr.children[1].index if curr!=None and curr.children[1]!=None else None)
        prev = curr
        for c in key:
            idx = ord(c) - ord('a')
            # print('idx: ', curr.index, curr.children[idx], curr.isLeaf)
            if curr.children[idx] is None:
                return curr.index
            prev = curr
            curr = curr.children[idx]
